...
- Surfaces are **siblings**, not nested (in the same keel)
- Each surface should use its corresponding layer for nested elements
- Surfaces provide the base for stacking layers

**Exception - Different Keels**:

- Modals, overlays, and popovers exist on a **different keel** (different z-index/stacking context)
...
// ❌ Wrong: Surface nested in surface (same keel)
<div className="bg-surface-1">
  <div className="bg-surface-2">
...
// ✅ Correct: Modal on different keel
<div className="bg-canvas">
  {/* Main page content */}
  <div className="bg-surface-1">
    Page content
...
```tsx
// ✅ Correct: Modal structure (different keel exception)
function PageWithModal() {
  return (
    <div className="bg-surface-1">
      ...
      {/* Modal - different keel, can use surface even with surface below */}
      {isModalOpen && (
        <div className="fixed inset-0 z-50">
          <div className="bg-backdrop fixed inset-0" />
          <div className="bg-surface-1 rounded-lg shadow-lg p-6">
...
OR
Is this a modal/overlay on a different keel (z-index)?
...
### ❌ Mistake 2: Nested Surfaces (Same Keel)

```tsx
// ❌ Wrong: Nested surfaces in same keel
<div className="bg-surface-1">
  <div className="bg-surface-2">
    Nested surface
  </div>
...
// ✅ Correct: Exception - Modal on different keel
<div className="bg-canvas">
  <div className="bg-surface-1">Page content</div>
  <div className="fixed inset-0 z-50">
    <div className="bg-surface-1">Modal (different keel)</div>
  </div>
</div>
```
...
2. **Use Surfaces for Pages and Top-Level Containers**: Surfaces are siblings, not nested (except modals/overlays on different keels)
...
- [ ] Surfaces are siblings, not nested (except modals/overlays on different keels)
