...
function Switch({ value, onChange, label, size = "sm", disabled, className }: IToggleSwitchProps) {
  return (
    <BaseSwitch.Root
      ...
      aria-label={label}
      className={cn(
        "relative inline-flex flex-shrink-0 cursor-pointer rounded-full border border-subtle transition-colors duration-200 ease-in-out " +
          // A switch is a control, so it has to show where the keyboard is.
