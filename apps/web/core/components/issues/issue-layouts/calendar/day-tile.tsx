...
import type { TGroupedIssues, TIssue, TIssueMap, TPaginationData, ICalendarDate } from "@keel/types";
// types
// ui
// components
import { cn, renderFormattedPayloadDate } from "@keel/utils";
    ...
    <>
      <div ref={dayTileRef} className="group relative flex h-full w-full flex-col">
        ...
        >
          ...
          {isToday ? (
            <span className="flex size-5 items-center justify-center rounded-full bg-accent-primary text-11 font-semibold text-on-accent tabular-nums shadow-raised-100">
              {date.date.getDate()}
            </span>
          ) : (
            <>{date.date.getDate()}</>
        ...
        >
          <div
            className={cn("flex size-6 items-center justify-center rounded-full", {
              "bg-accent-primary text-on-accent": isSelectedDate,
