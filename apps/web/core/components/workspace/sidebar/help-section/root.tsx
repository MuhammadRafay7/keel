...
import { KeelVersionNumber } from "@/components/global/version-number";
// hooks
  ...
  return (
    <>
      ...
      <CustomMenu
        ...
        closeOnSelect
      >
        <CustomMenu.MenuItem onClick={() => window.open("https://keel.ostenmark.com/docs", "_blank")}>
          <div className="flex items-center gap-x-2 rounded-sm text-11">
            <PageIcon className="h-3.5 w-3.5 text-secondary" height={14} width={14} />
            <span className="text-11">{t("documentation")}</span>
          </div>
        </CustomMenu.MenuItem>
        <CustomMenu.MenuItem onClick={() => window.open("mailto:sales@ostenmark.com", "_blank")}>
          <div className="flex items-center gap-x-2 rounded-sm text-11">
