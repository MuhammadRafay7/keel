...
import { renderKeelDocToPdfBuffer } from "@/lib/pdf";
...
describe("PDF Rendering Integration", () => {
  describe("renderKeelDocToPdfBuffer", () => {
    it("should render empty document to valid PDF", async () => {
      ...
      const buffer = await renderKeelDocToPdfBuffer(doc);
  ...
  describe("page options", () => {
    it("should support different page sizes and verify content renders", async () => {
      ...
      const a4Buffer = await renderKeelDocToPdfBuffer(doc, { pageSize: "A4" });
      const letterBuffer = await renderKeelDocToPdfBuffer(doc, { pageSize: "LETTER" });
    ...
    it("should support landscape orientation and verify content", async () => {
      ...
      const portraitBuffer = await renderKeelDocToPdfBuffer(doc, { pageOrientation: "portrait" });
      const landscapeBuffer = await renderKeelDocToPdfBuffer(doc, { pageOrientation: "landscape" });
  ...
  describe("metadata rendering", () => {
    it("should render user mentions with resolved display name", async () => {
      ...
      const buffer = await renderKeelDocToPdfBuffer(doc, { metadata });
      const text = await extractPdfText(buffer);
  ...
  describe("noAssets option", () => {
    it("should render text but skip images when noAssets is true", async () => {
      ...
      const buffer = await renderKeelDocToPdfBuffer(doc, { noAssets: true });
      const text = await extractPdfText(buffer);
