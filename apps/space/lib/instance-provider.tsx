...
// keel imports
import { SPACE_BASE_PATH } from "@keel/constants";
import { KeelLockup } from "@keel/propel/icons";
// assets
import KeelBackgroundPatternDark from "@/app/assets/auth/background-pattern-dark.svg?url";
import KeelBackgroundPattern from "@/app/assets/auth/background-pattern.svg?url";
// components
...
export const InstanceProvider = observer(function InstanceProvider({ children }: { children: React.ReactNode }) {
  ...
  const patternBackground = resolvedTheme === "dark" ? KeelBackgroundPatternDark : KeelBackgroundPattern;
          ...
          <div className="z-50 container mx-auto flex h-[110px] flex-shrink-0 items-center justify-between gap-5 px-5 lg:px-0">
            <div className="flex items-center gap-x-2 py-10">
              <Link to={`${SPACE_BASE_PATH}/`}>
                <KeelLockup className="h-7 w-auto text-primary" />
              </Link>
            </div>
          </div>
          <div className="absolute inset-0 z-0">
            <img src={patternBackground} className="h-full w-screen object-cover" alt="Keel background pattern" />
          </div>
